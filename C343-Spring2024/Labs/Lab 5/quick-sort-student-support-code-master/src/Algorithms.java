public class Algorithms {
     ...
     */
    public static <T> Iterator<T> contains(Sequence<T> S, T x) {
     ...
     */
    public static <T> boolean equal_sequences(Sequence<T> s1, Sequence<T> s2) {
     ...
     */
    public static <T extends Comparable<T>> T max_of_sequence(Sequence<T> s, T zero) {
        T m = zero;
        for (Iterator<T> i = s.begin(); ! i.equals(s.end()); i.advance()) {
            T c = i.get();
            if (m.compareTo(c) < 0)
